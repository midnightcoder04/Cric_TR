...
export default function PlayerForm() {
  ...
  // Load all India players (from all formats, deduplicated)
  useEffect(() => {
    async function load() {
      try {
  ...
  // Fetch form when selected or format changes
  useEffect(() => {
  ...
  // Build chart data: pivot form data into per-match rows
  // x-axis: match index (1-5), y-axis: impact_score per player
  const chartData = (() => {
    const byPlayer = {}
  ...
  // Build per-player recent match table
  const tableByPlayer = {}
  for (const p of selected) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        <TrendingUp className="text-pitch-400" size={22} />
        <h1 className="text-2xl font-bold text-white">Player Form</h1>
      </div>
        ...
        <div className="flex flex-wrap gap-2 min-h-[36px]">
          ...
          {selected.length < 5 && (
            <div className="relative">
              <div className="flex items-center gap-1.5 bg-slate-700/50 border border-slate-600/50 rounded-full px-3 py-1">
                <Search size={12} className="text-slate-400" />
                <input
                  className="bg-transparent outline-none text-sm text-slate-200 placeholder-slate-500 w-36"
              ...
              {showDropdown && search && filtered.length > 0 && (
                <div className="absolute top-full mt-1 left-0 z-20 bg-slate-800 border border-slate-700 rounded-xl shadow-xl w-56 overflow-hidden">
                  {filtered.map(p => (
                    <button
                      key={p}
                      onClick={() => addPlayer(p)}
                      className="w-full flex items-center gap-2 px-3 py-2 hover:bg-slate-700 text-sm text-slate-200 text-left"
                    >
                      <Plus size={12} className="text-pitch-400" />
                      {p}
                    </button>
                  ))}
                </div>
      ...
      {selected.length === 0 ? (
        ...
        <>
          ...
          <div className="card">
            ...
            {chartData.length === 0 ? (
              <p className="text-slate-500 text-sm py-8 text-center">No match data found for selected players / format</p>
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                  <XAxis dataKey="match" tick={{ fill: '#94a3b8', fontSize: 12 }} />
              ...
              return (
                <div key={p} className="card">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs text-slate-400">— Last {matches.length} {fmt === 'All' ? '' : fmt} matches</span>
                  </div>
                  {matches.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {matches.map((m, mi) => (
                            <tr key={mi} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                              <td className="py-2 text-slate-400">{m.date.slice(0, 10)}</td>
                              <td className="py-2">
                                <span className="text-xs font-medium text-slate-300 uppercase">{m.format}</span>
                              </td>
                              <td className="py-2 text-slate-400 max-w-[160px] truncate">{m.venue}</td>
                              <td className="py-2 text-right">
                                <span className="font-bold" style={{
                                  color: m.impact_score >= 70 ? '#22c55e' : m.impact_score >= 50 ? '#3b82f6' : m.impact_score >= 30 ? '#f59e0b' : '#ef4444'
                                }}>{m.impact_score}</span>
                              </td>
                              <td className="py-2 text-right text-slate-300">{m.bat_score}</td>
                              <td className="py-2 text-right text-slate-300">{m.bowl_score}</td>
                              <td className="py-2 text-right text-slate-300">{m.runs}</td>
                              <td className="py-2 text-right text-slate-300">{m.wickets}</td>
                              <td className="py-2 text-right text-slate-400">{m.strike_rate}</td>
                              <td className="py-2 text-right text-slate-400">{m.economy}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
